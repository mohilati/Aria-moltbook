...
def _gemini_request(model: str, system: str, user: str, max_output_tokens: int) -> str:
    ...
    if not candidates: raise RuntimeError(f"Gemini returned no candidates: {data}")
    cand = candidates[0]
    parts = ((cand.get("content") or {}).get("parts") or [])
    text = "\n".join(str(p.get("text","")).strip() for p in parts if isinstance(p,dict) and p.get("text")).strip()
    if not text:
        raise RuntimeError(f"Gemini returned no usable text (finishReason={cand.get('finishReason')}): {data}")
    return text

def llm_text(client: bool, system: str, user: str, max_output_tokens: int = 220) -> str:
    if not client or not GEMINI_API_KEY: raise RuntimeError("GEMINI_API_KEY is missing.")
    ...
    for model in models:
        for attempt in range(1,5):
            try:
                ...
                return _gemini_request(model, system, user, min(budget, 1400))
            except httpx.HTTPStatusError as e:
                last = e
                if e.response.status_code not in TEMPORARY_STATUS: break
                print(f"Gemini temporary error: model={model} HTTP {e.response.status_code}, attempt={attempt}/4 - {e.response.text[:500].replace(chr(10),' ')}")
                time.sleep(min(20, 2 ** (attempt-1)))
            except RuntimeError as e:
                last = e
                print(f"Gemini response problem: model={model}, attempt={attempt}/4 - {e}")
                time.sleep(min(12, 2 ** (attempt-1)))
    raise RuntimeError(f"All Gemini attempts failed: {last!r}")
        ...
        if suspension_record:
            ...
            if match:
                try:
                    ...
                    print("Could not parse suspension expiry; keeping publishing lock for safety.")
                    save_memory(memory)
                    return 0
            elif "suspended" in suspension_record.lower():
                print("Publishing disabled due to an unparsed Moltbook suspension record.")
                save_memory(memory)
                return 0
